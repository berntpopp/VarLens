...
describe('platform identity JWT validation', () => {
  ...
  test('rejects wrong environment audience', () => {
    const token = signJwt(basePayload('varlens-platform:app:varlens:test'))
...
describe('platform identity entitlement validation', () => {
  test('accepts the varlens-platform-operations wrapped entitlement decision contract', async () => {
...
describe('platform identity callback session state', () => {
  ...
  test('retries the identity flow once when TOTP enrollment lacks the OTP amr claim', async () => {
    ...
    expect(rejectedRetry.body).toContain('Sign-in could not be completed')
    await app.close()
  })
