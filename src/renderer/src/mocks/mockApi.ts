  ...
  variants: {
    query: async (caseId, filters, cursor, limit = 50) => {
      ...
      if (filters.clinvars !== undefined && filters.clinvars.length > 0) {
        filtered = filtered.filter((v) => filters.clinvars!.includes(v.clinvar ?? ''))
      }
      if (filters.gnomad_af_max !== undefined) {
        filtered = filtered.filter((v) => (v.gnomad_af ?? 0) <= filters.gnomad_af_max!)
      }
      if (filters.cadd_min !== undefined) {
        filtered = filtered.filter((v) => (v.cadd ?? 0) >= filters.cadd_min!)
      }
